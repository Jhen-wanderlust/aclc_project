                  ...
                  <h1 class="cat-title">{{ cat }}</h1>
                   <div class="service-info" v-for="(service,index) in services" v-if="cat == service.category">
                      <label>
                        ...
                        <span class="service-info"> {{ service.name }} : php {{ service.price }} with {{ service.discount }} % off  <br> Price: <br> php {{ discountedPrice(service) }} </span>
...
<!--Services-->
              ...
              <span v-show="!afterSubmit" class="total-pay">{{ totalPay | somma }}</span> 
                <form v-on:submit.prevent>
                 ...
                 <input @click="changeDate" type="text" style="z-index: 1003; display: block; opacity: 1; top: 4%; transform: scaleX(1) scaleY(1);" class="datepicker" v-model.lazy="Schedule" placeholder="Pick A Date" required>
                    ...
                    if(this.date)
                     {
                       ...
                       this.$http.get('https://salon-management-system-598d0.firebaseapp.com/jheny-app/appointmentReadable.json').then(function(data)
                         {
                  ...
                  if(this.validationDate == true & this.validationName == true & this.validationTime == true & this.validationService == true & this.validationNumber == true)
                    {
                      ...
                      this.$http.post('https://reservation-system-69ff9.firebaseio.com/jheny-app/appointments.json',
                      {
                                 "type" : "walkin",
                        ...
                        {
                            this.$http.post('https://reservation-system-69ff9.firebaseio.com/jheny-app/appointmentReadable.json',
                                          {
                                                     "date" : this.date,
                                                     "time" : time,
                                          }).then(function(data)
            ...
            fetcher()
             {
                 ...
                 this.$http.get('https://reservation-system-69ff9.firebaseio.com/jheny-app/services.json').then(function(data)
                  {
                       return data.json();
                  }).then(function(data)
                  {
