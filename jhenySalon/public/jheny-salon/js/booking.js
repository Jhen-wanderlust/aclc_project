...
<!--Services-->
              ...
              <div v-if="!afterSubmit" class="service-main-container">
                <div class="animated zoomIn main-service" v-for="cat in categories">
                  ...
                  <div class="service-info" v-for="(service,index) in services" :key="index" v-if="cat == service.category">
                  <label>
                    <input v-model="totalPay" :value=discountedPrice(service)  @click="getService(val)" type="checkbox" class="filled-in" :id="service.unique">
                    <span class="service-info"> {{ service.name }} : php {{ service.price }}  {{ service.discount }} {{ service.discount_symbol }} <br>Price: php {{ discountedPrice(service) }} </span>
                  </label>
